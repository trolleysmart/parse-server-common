...
ProductPriceService.buildSearchQuery = function(criteria) {
  ...
  _microBusinessParseServerCommon.ServiceBase.addNumberSearchToQuery(conditions, query, 'priceToDisplay', 'priceToDisplay');
  _microBusinessParseServerCommon.ServiceBase.addNumberSearchToQuery(conditions, query, 'saving', 'saving');
  _microBusinessParseServerCommon.ServiceBase.addNumberSearchToQuery(conditions, query, 'savingPercentage', 'savingPercentage');
  _microBusinessParseServerCommon.ServiceBase.addDateTimeSearchToQuery(conditions, query, 'offerEndDate', 'offerEndDate');

  if (conditions.has('status')) {
    var value = conditions.get('status');
  ...
  if (conditions.has('tag')) {
    var _value2 = conditions.get('tag');

    if (_value2) {
      query.equalTo('tags', _value2);
    }
  }

  if (conditions.has('tags')) {
    var _value3 = conditions.get('tags');

    if (_value3) {
      query.containedIn('tags', _value3.toArray());
    }
  }

  if (conditions.has('tagId')) {
    var _value4 = conditions.get('tagId');

    if (_value4) {
      query.equalTo('tags', _schema.Tag.createWithoutData(_value4));
    }
  }

  if (conditions.has('tagIds')) {
    var _value5 = conditions.get('tagIds');

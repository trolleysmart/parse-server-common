...
var MasterProductPriceService = (function(_ServiceBase) {
  ...
  function MasterProductPriceService() {
    ...
    return _possibleConstructorReturn(
      this,
      (MasterProductPriceService.__proto__ || Object.getPrototypeOf(MasterProductPriceService)).apply(this, arguments),
    );
  }
            ...
            switch ((_context2.prev = _context2.next)) {
              case 0:
                ...
                return _microBusinessParseServerCommon.ParseWrapperService
                  .createQuery(_schema.MasterProductPrice)
                  .equalTo('objectId', id)
                  .limit(1)
                  .find();
...
MasterProductPriceService.buildSearchQuery = function(criteria) {
  ...
  if (conditions.has('storeIds')) {
    ...
    var queryWithoutStandardCriteriaAdded = _microBusinessParseServerCommon.ParseWrapperService.createOrQuery(
      storeIds.map(function(storeId) {
        return MasterProductPriceService.buildSearchQueryInternal(criteria.setIn(['conditions', 'storeId'], storeId));
      }),
    );

    var query = _microBusinessParseServerCommon.ParseWrapperService.addStandardCriteriaToQuery(
      _schema.MasterProductPrice,
      queryWithoutStandardCriteriaAdded,
      criteria,
    );

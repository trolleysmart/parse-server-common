...
export function createCrawlResultInfo(crawlSessionId) {
  return Map({
    crawlSessionId: crawlSessionId || createCrawlSession()
      .getId(),
    resultSet: Map({
      price: uuid(),
    }),
...
function expectCrawlResultInfo(crawlSessionPriceInfo, expectedCrawlResultInfo) {
  expect(crawlSessionPriceInfo.get('crawlSession')
      .getId())
    .toBe(expectedCrawlResultInfo.get('crawlSessionId'));
  expect(crawlSessionPriceInfo.get('crawlSessionId'))
    .toBe(expectedCrawlResultInfo.get('crawlSessionId'));
  expect(crawlSessionPriceInfo.get('resultSet'))
    .toEqual(expectedCrawlResultInfo.get('resultSet'));
}

describe('constructor', () => {
  test('should set class name', () => {
    expect(createCrawlResult()
        .className)
      .toBe('CrawlResult');
  });
});
